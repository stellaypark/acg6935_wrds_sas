...
%macro ind_ff12 (dset, outp, sic, bin_var, ind_code, ff12_name=ff12_name );
...
*12 Other  Other -- Mines, Constr, BldMt, Trans, Hotels, Bus Serv, Entertainment;
	...
	if(&ind_code=11) then &bin_var.12=1;		else &bin_var.12=0;

	&ff12_name = "                        ";
	if(&ind_code=1) then &ff12_name = "Consumer nondurables";
	if(&ind_code=2) then &ff12_name = "Consumer durables";
	if(&ind_code=3) then &ff12_name = "Manufacturing";
	if(&ind_code=4) then &ff12_name = "Energy";
	if(&ind_code=5) then &ff12_name = "Chemicals";
	if(&ind_code=6) then &ff12_name = "Business equipment";
	if(&ind_code=7) then &ff12_name = "Telephone and television";
	if(&ind_code=8) then &ff12_name = "Utilities";
	if(&ind_code=9) then &ff12_name = "Wholesale, retail";
	if(&ind_code=10) then &ff12_name = "Healthcare";
	if(&ind_code=11) then &ff12_name = "Finance";
	if(&ind_code=12) then &ff12_name = "Other";
